...
export function PromptSidebar() {
  ...
  const onCreateFolder = useCallback(
    () => setFolderSheet({ open: true }),
    [setFolderSheet],
  );
  const onEditFolder = useCallback(
    (folder: Folder) => setFolderSheet({ open: true, folder }),
    [setFolderSheet],
  );
  const onDeleteFolder = useCallback(
    (folder: Folder) => setDeleteFolderDialog({ open: true, folder }),
    [setDeleteFolderDialog],
  );
  const onCreatePrompt = useCallback(
    (folderId?: string) => setPromptSheet({ open: true, folderId }),
    [setPromptSheet],
  );
  const onEditPrompt = useCallback(
    (prompt: Prompt) => setPromptSheet({ open: true, prompt }),
    [setPromptSheet],
  );
  const onDeletePrompt = useCallback(
    (prompt: Prompt) => setDeletePromptDialog({ open: true, prompt }),
    [setDeletePromptDialog],
  );
  ...
  return (
    <DragDropProvider
      ...
      onDragEnd={(event) => {
        ...
        const promptId = sourceId.startsWith("prompt-")
          ? sourceId.slice(7)
          : null;
        if (!promptId) return;
    ...
    >
      <div className="flex h-full flex-col">
        ...
        <ScrollArea
          ...
          viewportClassName="no-table viewport-table-height-full"
        >
          <div className="flex flex-col p-2 px-3">
            {filteredData.folders.length === 0 &&
            filteredData.rootPrompts.length === 0 ? (
              ...
              <>
                {filteredData.folders.map((folder) => (
                  <DroppableFolder
                    ...
                    folder={folder}
                    prompts={
                      filteredData.promptsByFolder.get(folder.id) ||
                      promptsByFolder.get(folder.id) ||
                      []
                    }
...
function RootDropZone({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "min-h-[8px] grow rounded-sm transition-colors",
        ...
        {showActions && (
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              className="bg-card absolute top-1/2 right-2 -translate-y-1/2"
            >
              <Button
                variant="ghost"
                size="icon"

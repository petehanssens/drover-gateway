...
Any custom backend for vector store should implement the `VectorStore` interface. The interface is defined in [vectorstore/store.go](https://github.com/petehanssens/drover-gateway/blob/main/framework/vectorstore/store.go).

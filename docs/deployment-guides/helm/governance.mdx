...
| `scope_kind` | Yes | Scope level: `global`, `provider`, `provider_key`, `virtual_key`, `virtual_key_provider`, `virtual_key_provider_key` |
...
| `request_types` | No | Request types this override applies to (e.g. `chat_completion`) |
| `pricing_patch` | Yes | JSON-encoded pricing fields to override |

```yaml
bifrost:
  governance:
    pricingOverrides:
      # Override pricing for a specific model globally
      ...
      # Override pricing for a specific provider key

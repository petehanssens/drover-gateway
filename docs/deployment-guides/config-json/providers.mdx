...
```json
{
  "providers": {
    ...
    "fireworks": {
      "keys": [{ "name": "fireworks-main", "value": "env.FIREWORKS_API_KEY", "models": ["*"], "weight": 1.0 }]
    },
    "parasail": {
      "keys": [{ "name": "parasail-main", "value": "env.PARASAIL_API_KEY", "models": ["*"], "weight": 1.0 }]
    },
    "elevenlabs": {
      "keys": [{ "name": "elevenlabs-main", "value": "env.ELEVENLABS_API_KEY", "models": ["*"], "weight": 1.0 }]
    },
    "runway": {
      "keys": [{ "name": "runway-main", "value": "env.RUNWAY_API_KEY", "models": ["*"], "weight": 1.0 }]
    }
  }
}
```
